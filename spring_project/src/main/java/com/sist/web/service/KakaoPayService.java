...
public class KakaoPayService {
	...
	public KakaoPayReadyResponse ready (KakaoPayReadyRequest kakaoPayReadyRequest) {
		...
		if (kakaoPayReadyRequest != null) {
			...
			ResponseEntity<KakaoPayReadyResponse> responseEntity = 
					restTemplate.postForEntity(KAKAOPAY_READY_URL, requestEntity, KakaoPayReadyResponse.class);
	...
	public KakaoPayApproveResponse approve (KakaoPayApproveRequest kakaoPayApproveRequest) {
		...
		if (kakaoPayApproveRequest != null) {
			...
			if (!StringUtil.isEmpty(kakaoPayApproveRequest.getTotal_amount()) &&
					kakaoPayApproveRequest.getTotal_amount() > 0) {
	...
	public KakaoPayCancelResponse cancel (KakaoPayCancelRequest kakaoPayCancelRequest) {
		...
		if (kakaoPayCancelRequest != null) {
			...
			if (!StringUtil.isEmpty(kakaoPayCancelRequest.getCid_secret())) {
				parameters.put("cid_secret", kakaoPayCancelRequest.getCid_secret());
			}
			
			parameters.put("tid", kakaoPayCancelRequest.getTid());
			parameters.put("cancel_amount", kakaoPayCancelRequest.getCancel_amount());
			parameters.put("cancel_tax_free_amount", kakaoPayCancelRequest.getCancel_tax_free_amount());
			...
			ResponseEntity<KakaoPayCancelResponse> responseEntity =
					restTemplate.postForEntity(KAKAOPAY_CANCEL_URL, requestEntity, KakaoPayCancelResponse.class);
